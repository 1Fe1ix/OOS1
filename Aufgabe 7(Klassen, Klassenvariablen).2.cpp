...
static int _s_count; static int _s_id_generator; std::string _name; std::string _location;
int _age; double _business_done; int _transaction_count; int _id;
...
Die statischen Variablen '_s_count' und '_s_id_generator' werden automatisch inkrementiert,

wenn ein Objekt der Klasse 'Customer' angelegt wird. Verfassen Sie

a) einen Konstruktor mit Default-Parametern für '_name', '_location' und '_age'.
...
class Customer {
	static int _s_count;		//Auto inkrement
	static int _s_id_generator;	//Auto inkrement wenn ein Objekt der Klasse Coustmer angelegt wird 
	std::string _name;
	std::string _location;
	int _age;
	double _business_done = 0;    //Gibt an welcher umsatz bei einem Einkauf gemacht wurde
	int _transaction_count = 0;
	int _id;				  //Kundennummer in abhängigkeit zu _s_id_generator

public:
	Customer(std::string name = "Meier", int age = 28, std::string location = "Esslingen")
	{
		_name = name;
		_age = age;
		_location = location;
		//Zählt die kunden Objektübergreifend 
		//beginnend bei 0
		_s_count++;
		//_s_id_generator speichert den wert Objektübergreifend 
		//so bekommen die objekte nacheinander aufsteigend ihre ID
		//beginnend mit 1
		_id = ++_s_id_generator ;
	}
	...
	void do_business(double amount)
	{
		...
		//Eigentliche rechenoperation der Gesamtsumme der Transaktionen 
