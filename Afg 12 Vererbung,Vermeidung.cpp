/* Afg 12 Vererbung,Vermeidung.cpp +
...
die eigenen Speicherplatz in Anspruch nehmen. 
In der Ausgabe oben werden daher die Instanzvariablen a und b mehrfach an D1 und D2 vererbt. 
...
Es soll also für das Attribut a ein gemeinsames Erbgut über alle abgeleiteten Klassen geben und für das Attribut b ein gemeinsames 
Erbgut für die Klassen C2 und C3 aber ein separates Erbgut für die Klasse C1.
Verwenden Sie dazu virtuelle Ableitung und ergänzen Sie Initialisierungslisten wenn notwendig. 
Ändern Sie nicht die Default-Parameter der Konstruktoren und ändern sie nicht die existierende Delegation an Konstruktoren 
...
public:
	D1(int c2_a = 0xD1C2A, int c3_a = 0xD1C3A,
		...
		: A(0xD1A),C2(c2_a, c2_b), C3(c3_a, c3_b){} //A mit passendem Inizialisierungswert in Delegation
	void print() {
		cout << "D1::print() ..." << endl;
...
public:
	...
	D2(int c1_a = 0xD2C1A, int c2_a = 0xD2C2A, int c3_a = 0xD2C3A,
		...
		: B(0x0,0xD2B), C1(c1_a, c1_b), C2(c2_a, c2_b), C3(c3_a, c3_b) {}  /// Was ist das Problem?
